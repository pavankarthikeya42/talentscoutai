...
export function ApplicationsPage() {
  ...
  const { data: jobsData } = useQuery({
    queryKey: ['jobs', { page_size: 50 }],
    queryFn: () => jobsApi.list({ page_size: 50 }).then((r) => r.data),
  })

  const { data, isLoading } = useQuery({
    queryKey: ['applications', { jobFilter, statusFilter, page }],
    queryFn: () =>
      screeningApi.getAllApplications({
  ...
  const statusMutation = useMutation({
    mutationFn: ({ appId, status }: { appId: string; status: string }) =>
      screeningApi.updateStatus(appId, status),
    onMutate: async ({ appId, status }) => {
      await qc.cancelQueries({ queryKey: ['applications'] })
      const previousData = qc.getQueryData(['applications', { jobFilter, statusFilter, page }])
      qc.setQueryData(
        ['applications', { jobFilter, statusFilter, page }],
        (old: any) => {
          if (!old) return old
          return {
            ...old,
    ...
    onError: (err, variables, context: any) => {
      if (context?.previousData) {
        qc.setQueryData(['applications', { jobFilter, statusFilter, page }], context.previousData)
      }
    ...
    <div className="space-y-6 page-enter">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {total} total · candidates with score ≥ {SCORE_THRESHOLD} can be scheduled for interview
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <Select value={jobFilter} onValueChange={(v) => { setJobFilter(v); setPage(1) }}>
          <SelectTrigger className="w-52"><SelectValue placeholder="All Jobs" /></SelectTrigger>
          <SelectContent>
      ...
      {isLoading ? <PageLoader /> : apps.length === 0 ? (
        <EmptyState
          ...
          description="Run AI screening on a job to populate applications here"
        />
      ) : (
        <Card>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    ...
                    {isHR && <th className="text-left px-4 py-3 font-medium text-gray-500">Change Status</th>}
                    {isHR && <th className="text-left px-4 py-3 font-medium text-gray-500">Action</th>}
                  </tr>
                </thead>
                <tbody>
                  {apps.map((app: Application) => (
                    <tr key={app.id} className="border-b border-gray-50 table-row-hover">
                      <td className="px-4 py-3 font-medium text-gray-900">
                        <Link to={`/candidates/${app.candidate_id}`} className="hover:text-blue-600">
                          {app.candidate_name ?? '—'}
                        </Link>
                      </td>
                      <td className="px-4 py-3 text-gray-500">
                        <Link to={`/jobs/${app.job_id}`} className="hover:text-blue-600">
                          {app.job_title ?? '—'}
                        </Link>
                      </td>
                      <td className="px-4 py-3">
                        {app.suitability_score != null ? (
                          <span className={`font-semibold ${
                            app.suitability_score >= 70 ? 'text-green-600'
                            : app.suitability_score >= SCORE_THRESHOLD ? 'text-yellow-600'
                            : 'text-red-500'
                          }`}>
                      ...
                      {isHR && (
                        <td className="px-4 py-3">
                          <Select
                            value={app.status}
                            onValueChange={(v) => statusMutation.mutate({ appId: app.id, status: v })}
                          >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex flex-wrap items-center gap-2">
                              {(app.suitability_score ?? 0) >= SCORE_THRESHOLD ? (
                                <Button
                                  size="sm"
                                  variant="outline"
                                  ...
                                  onClick={() => {
                                    setSchedulingApp({
                                      ...
                                      interviewerId: app.recruiter_id ?? '',
                                    })
                                  }}
                                >
                              ...
                              {['shortlisted', 'interview', 'offered', 'hired', 'rejected'].includes(app.status) && (
      ...
      <ScheduleInterviewModal
        ...
        defaultInterviewerId={schedulingApp?.interviewerId}
      />
    </div>
  )
}
